...
public class Miner
{
	...
	public static void handle_input(Arena arena, Smartrix smx_miner, int line, InputActions ia)
	{
	...
	public static void perform_special_attack(Arena arena, Smartrix sm_miner, int miner_id, int special_variant)
	{
		...
		} else
		{
			...
			// TODO: 24.02.23 for now there is only one crystal, later 3 will be introduced

			// TODO: 24.02.23 for now the special attacks are a switch case here, maybe this will move to a separate function in the MinerAttack enum
			switch (ma)
			{
				case IMPACT:
				{
					...
					arena.spawn_particle(miner_tx + Config.CONF.IMPACTOR_TARGET_DISTANCE.value * Util.fourdirx[view_dir], arena.sm_miners.get(0, MinerData.TILEY.ordinal()) + Config.CONF.IMPACTOR_TARGET_DISTANCE.value * Util.fourdiry[view_dir], Particles.TYPE_IMPACTOR);
				}
				break;
	...
	private static void move(Arena arena, Smartrix smx_miner, int line, int dir_move)
	{
		...
		if (arena.free_tile(next_tx, next_ty))
		{
